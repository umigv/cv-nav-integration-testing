...
def on_message(client, userdata, msg):
    ...
    occ = np.array(json.loads(msg.payload.decode()))
    np.savetxt('testing_mask_current.txt', occ, fmt='%d')
    # occ_rows, occ_cols = occ.shape[:2] 
    # transformed_image, bottomLeft, bottomRight, topRight, topLeft, maxWidth, maxHeight  = getBirdView(occ_rows, occ_cols, ZED)
    transformed_image, bottomLeft, bottomRight, topRight, topLeft, maxWidth, maxHeight  = getBirdView(occ, ZED)

    #Transform video to bird's eye view
    # transform_vid = getBirdView(376, 672, ZED)
